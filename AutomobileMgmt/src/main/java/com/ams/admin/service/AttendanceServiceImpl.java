...
import com.ams.admin.dao.AttendanceDao;
...
public class AttendanceServiceImpl implements AttendanceService {
	...
	public List<EmployeeAttendance> searchEmployee(EmployeeAttendance employeeAttendance) {
		if (employeeAttendance.getEmployeeId().equals(null) || (employeeAttendance.getEmployeeId().equals(""))) {
			return attendanceDao.searchEmployeeByName(employeeAttendance);
		} else if (employeeAttendance.getEmployeeName().equals(null) || employeeAttendance.getEmployeeName().equals("")) {
			return attendanceDao.searchEmployeeById(employeeAttendance);
		}
		return attendanceDao.searchEmployee(employeeAttendance);
	}
	...
	public List searchAttendance(SearchAttendance searchAttendance) {
		...
		/**
		 * Calculate the leaves taken by employee in specified time values by Admin.
		 * Also, restrict the to_date value exceeding current date.
		 */
		int i = 0;
	...
	public List employeeSearchAttendance(SearchAttendance searchAttendance, String employeeId) {
		...
		/**
		 * Calculate the leaves taken by employee in specified time values by Employee.
		 * Also, restrict the to_date value exceeding current date.
		 */
		int i = 0;
		int listSize = list.size();
